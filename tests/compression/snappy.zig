...
test "tiny Snappy blocks round trip before and after table reuse" {
    const workspace = try bedwire.FlateWorkspace.create(std.testing.allocator, 128);
    defer workspace.destroy();
    var output: [16]u8 = undefined;
    for ([_][]const u8{ "", "a", "ab", "abc", "abcdabcd", "abc", "", "abcdabcd" }) |input| {
        const encoded = try bedwire.snappy.compress(input, workspace.output, &workspace.snappy_table);
        try std.testing.expectEqualSlices(u8, input, try bedwire.snappy.decompress(encoded, &output, .{}));
    }
}
...
test "Snappy copy forms accept adjacent and separated source ranges" {
    ...
    for ([_][]const u8{
        ...
        try std.testing.expectEqualStrings("abcdabcd", try bedwire.snappy.decompress(encoded, &output, .{}));
    }
    try std.testing.expectEqualStrings("abcdab", try bedwire.snappy.decompress(&.{ 6, 12, 'a', 'b', 'c', 'd', 6, 4, 0 }, &output, .{}));
}

test "raw Snappy compresses repetitive bytes and round trips arbitrary data" {
    const workspace = try bedwire.FlateWorkspace.create(std.testing.allocator, 8192);
    defer workspace.destroy();
    ...
    for (0..128) |i| {
        ...
        const len = i * 16;
        const encoded = try bedwire.snappy.compress(input[0..len], workspace.output, &workspace.snappy_table);
        if (i > 1 and i % 2 == 0) try std.testing.expect(encoded.len < len);
        try std.testing.expectEqualSlices(u8, input[0..len], try bedwire.snappy.decompress(encoded, &output, .{}));
        for (0..encoded.len) |end| {
            if (bedwire.snappy.decompress(encoded[0..end], &output, .{})) |_| return error.AcceptedTruncation else |_| {}
        }
    }
}
...
test "all Snappy copy tags, overlap, malformed offsets, and output atomicity" {
    ...
    for ([_][]const u8{ &.{ 7, 8, 'x', 'a', 'b', 1, 2 }, &.{ 7, 8, 'x', 'a', 'b', 14, 2, 0 }, &.{ 7, 8, 'x', 'a', 'b', 15, 2, 0, 0, 0 } }) |encoded| {
        try std.testing.expectEqualStrings("xababab", try bedwire.snappy.decompress(encoded, &output, .{}));
    }
    for ([_][]const u8{ &.{ 5, 0, 'a', 1, 0 }, &.{ 5, 0, 'a', 1, 2 }, &.{ 4, 0, 'a', 1, 1 } }) |encoded| {
        @memset(&output, 0xaa);
        try std.testing.expectError(error.InvalidCompressedData, bedwire.snappy.decompress(encoded, &output, .{}));
        try std.testing.expectEqualSlices(u8, &([_]u8{0xaa} ** 32), &output);
    }
    try std.testing.expectError(error.LimitExceeded, bedwire.snappy.decompress(&.{ 0xff, 0xff, 0xff, 0xff, 0x0f }, &output, .{}));
    try std.testing.expectError(error.NonCanonicalVarInt, bedwire.snappy.decompress(&.{ 0x80, 0 }, &output, .{}));
    try std.testing.expectError(error.VarIntOverflow, bedwire.snappy.decompress(&.{ 0xff, 0xff, 0xff, 0xff, 0x10 }, &output, .{}));
    try std.testing.expectError(error.LimitExceeded, bedwire.snappy.decompress(&.{ 7, 8, 'x', 'a', 'b', 1, 2 }, &output, .{ .max_decompressed_batch_bytes = 6 }));
}
...
test "bounded random parser inputs cannot modify output on failure" {
    ...
    for (0..10000) |i| {
        ...
        if (bedwire.snappy.decompress(bytes, &output, .{ .max_decompressed_batch_bytes = 128 })) |decoded| {
            try std.testing.expect(decoded.len <= output.len);
        } else |_| {
            for (output) |byte| try std.testing.expectEqual(@as(u8, 0xaa), byte);
        }
        bedwire.batch.validate(bytes, .{ .max_decompressed_batch_bytes = 128, .max_packets_per_batch = 16 }) catch {};
    }
}
...
test "extended Snappy literals and copy offsets beyond 64 KiB" {
    ...
    for ([_]usize{ 60, 61, 256, 257, 65536, 65537 }) |len| {
        var writer = bedwire.protocol.Writer.init(input);
        ...
        writer.cursor += len;
        const decoded = try bedwire.snappy.decompress(writer.written(), output, .{});
