...
test "raw Snappy compresses repetitive bytes and round trips arbitrary data" {
    const workspace = try n.FlateWorkspace.create(std.testing.allocator, 8192);
    ...
    for (0..128) |i| {
        ...
        const len = i * 16;
        const encoded = try n.snappy.compress(input[0..len], workspace.output, &workspace.snappy_table);
        if (i > 1 and i % 2 == 0) try std.testing.expect(encoded.len < len);
        try std.testing.expectEqualSlices(u8, input[0..len], try n.snappy.decompress(encoded, &output, .{}));
        for (0..encoded.len) |end| {
            if (n.snappy.decompress(encoded[0..end], &output, .{})) |_| return error.AcceptedTruncation else |_| {}
        }
    }
}
...
test "all Snappy copy tags, overlap, malformed offsets, and output atomicity" {
    ...
    for ([_][]const u8{ &.{ 7, 8, 'x', 'a', 'b', 1, 2 }, &.{ 7, 8, 'x', 'a', 'b', 14, 2, 0 }, &.{ 7, 8, 'x', 'a', 'b', 15, 2, 0, 0, 0 } }) |encoded| {
        try std.testing.expectEqualStrings("xababab", try n.snappy.decompress(encoded, &output, .{}));
    }
    for ([_][]const u8{ &.{ 5, 0, 'a', 1, 0 }, &.{ 5, 0, 'a', 1, 2 }, &.{ 4, 0, 'a', 1, 1 } }) |encoded| {
        @memset(&output, 0xaa);
        try std.testing.expectError(error.InvalidCompressedData, n.snappy.decompress(encoded, &output, .{}));
        try std.testing.expectEqualSlices(u8, &([_]u8{0xaa} ** 32), &output);
    }
    try std.testing.expectError(error.LimitExceeded, n.snappy.decompress(&.{ 0xff, 0xff, 0xff, 0xff, 0x0f }, &output, .{}));
    try std.testing.expectError(error.NonCanonicalVarInt, n.snappy.decompress(&.{ 0x80, 0 }, &output, .{}));
    try std.testing.expectError(error.VarIntOverflow, n.snappy.decompress(&.{ 0xff, 0xff, 0xff, 0xff, 0x10 }, &output, .{}));
    try std.testing.expectError(error.LimitExceeded, n.snappy.decompress(&.{ 7, 8, 'x', 'a', 'b', 1, 2 }, &output, .{ .max_decompressed_batch_bytes = 6 }));
}
...
test "bounded random parser inputs cannot modify output on failure" {
    ...
    for (0..10000) |i| {
        ...
        if (n.snappy.decompress(bytes, &output, .{ .max_decompressed_batch_bytes = 128 })) |decoded| {
            try std.testing.expect(decoded.len <= output.len);
        } else |_| {
            for (output) |byte| try std.testing.expectEqual(@as(u8, 0xaa), byte);
        }
        n.batch.validate(bytes, .{ .max_decompressed_batch_bytes = 128, .max_packets_per_batch = 16 }) catch {};
    }
}
...
test "extended Snappy literals and copy offsets beyond 64 KiB" {
    ...
    for ([_]usize{ 60, 61, 256, 257, 65536, 65537 }) |len| {
        var writer = n.protocol.Writer.init(input);
        ...
        writer.cursor += len;
        const decoded = try n.snappy.decompress(writer.written(), output, .{});
