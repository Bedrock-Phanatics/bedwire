...
test "independent Go raw compression, P384 and continuous AES-CTR fixtures" {
    ...
    try std.testing.expectEqualSlices(u8, expected, try zigrock.snappy.decompress(snappy, &output, .{}));
    const workspace = try zigrock.FlateWorkspace.create(std.testing.allocator, 4096);
    ...
    var crypto = zigrock.SessionCrypto.init(@splat(0x42));
    defer crypto.deinit();
    ...
    scalar_a[47] = 1;
    scalar_b[47] = 2;
    const a = try zigrock.spki.Ecdsa.KeyPair.fromSecretKey(try .fromBytes(scalar_a));
    const b = try zigrock.spki.Ecdsa.KeyPair.fromSecretKey(try .fromBytes(scalar_b));
    const derived = try zigrock.ecdh.derive(a.secret_key, b.public_key, @splat(3));
