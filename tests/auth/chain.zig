...
test "OIDC tokens are bound to key id, issuer, audience and client key" {
    const allocator = testing.allocator;
    const signer = try support.deterministicKey(4);
    const client = try support.deterministicKey(5);

    const payload = try std.fmt.allocPrint(
        allocator,
        "{{\"exp\":200,\"iss\":\"issuer\",\"aud\":\"audience\",\"cpk\":\"{s}\",\"xname\":\"Steve\"," ++
            "\"mid\":\"" ++ support.chain_uuid ++ "\",\"xid\":\"1234\"}}",
        .{support.encodedKey(client.public_key)},
    );
    defer allocator.free(payload);

    const token = try support.signToken(allocator, signer, "{\"alg\":\"ES384\",\"kid\":\"one\"}", payload);
    defer allocator.free(token);

    const client_data = try support.buildClientData(allocator, client);
    defer allocator.free(client_data);

    const known: []const auth.OidcKey = &.{.{ .kid = "one", .key = signer.public_key }};
    var policy: auth.OidcPolicy = .{ .now = 100, .issuer = "issuer", .audience = "audience", .keys = known };

    var identity = try auth.verifyOidc(allocator, token, client_data, policy, support.limits);
    defer identity.deinit();
    try testing.expectEqualStrings("Steve", identity.display_name);
    try testing.expect(identity.online);

    policy.audience = "wrong";
    try testing.expectError(error.InvalidClaims, auth.verifyOidc(allocator, token, client_data, policy, support.limits));
    ...
    policy.now = 300;
    try testing.expectError(error.ExpiredToken, auth.verifyOidc(allocator, token, client_data, policy, support.limits));

    policy.now = 100;
    policy.keys = &.{};
    try testing.expectError(error.UnknownKey, auth.verifyOidc(allocator, token, client_data, policy, support.limits));

    policy.keys = &.{
        .{ .kid = "one", .key = signer.public_key },
        .{ .kid = "one", .key = client.public_key },
    };
    try testing.expectError(error.AmbiguousKey, auth.verifyOidc(allocator, token, client_data, policy, support.limits));

    policy.keys = known;
    const forged = try support.buildClientData(allocator, signer);
    defer allocator.free(forged);
    try testing.expectError(error.InvalidSignature, auth.verifyOidc(allocator, token, forged, policy, support.limits));
}

test "an OIDC token signed by an unrelated key is refused" {
    const allocator = testing.allocator;
    const signer = try support.deterministicKey(4);
    const impostor = try support.deterministicKey(6);
    const client = try support.deterministicKey(5);
    ...
    const token = try support.signToken(allocator, impostor, "{\"alg\":\"ES384\",\"kid\":\"one\"}", payload);
    defer allocator.free(token);

    const client_data = try support.buildClientData(allocator, client);
    defer allocator.free(client_data);

    const policy: auth.OidcPolicy = .{
        .now = 100,
        .issuer = "issuer",
        .audience = "audience",
        .keys = &.{.{ .kid = "one", .key = signer.public_key }},
    };

    try testing.expectError(error.InvalidSignature, auth.verifyOidc(allocator, token, client_data, policy, support.limits));
}

test "a non-numeric XUID is refused for online identities" {
    const allocator = testing.allocator;
    const signer = try support.deterministicKey(4);
    const client = try support.deterministicKey(5);
    ...
    const payload = try std.fmt.allocPrint(
        allocator,
        "{{\"exp\":200,\"iss\":\"issuer\",\"aud\":\"audience\",\"cpk\":\"{s}\",\"xname\":\"Steve\"," ++
            "\"mid\":\"" ++ support.chain_uuid ++ "\",\"xid\":\"not-a-number\"}}",
        .{support.encodedKey(client.public_key)},
    );
    defer allocator.free(payload);
