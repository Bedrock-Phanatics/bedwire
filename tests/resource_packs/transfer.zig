...
fn metadata() zigrock.transfer.Metadata {
...
test "metadata and chunks round trip with exact SHA256 and no archive allocation" {
    ...
    const decoded = try zigrock.transfer.Metadata.decode(encoded, .{});
    try std.testing.expectEqual(m.size, decoded.size);
    for (0..encoded.len) |end| {
        if (zigrock.transfer.Metadata.decode(encoded[0..end], .{})) |_| return error.AcceptedTruncation else |_| {}
    }
    var transfer = try zigrock.transfer.Transfer.init(std.testing.allocator, m, .{});
    defer transfer.deinit();
    while (!transfer.complete) {
        const request = try transfer.request();
        const request_bytes = try request.encode(&bytes, .{});
        const decoded_request = try zigrock.transfer.Request.decode(request_bytes, .{});
        const chunk = try zigrock.transfer.Transfer.serve(m, archive, decoded_request, .{});
        const chunk_bytes = try chunk.encode(&bytes, .{});
        try transfer.accept(try zigrock.transfer.Chunk.decode(chunk_bytes, .{}));
    }
...
test "resource transfer rejects bad count, chunk order, offsets, and final hash" {
    ...
    for (0..4) |case| {
        var transfer = try zigrock.transfer.Transfer.init(std.testing.allocator, m, .{});
        defer transfer.deinit();
        var chunk = try zigrock.transfer.Transfer.serve(m, archive, try transfer.request(), .{});
        switch (case) {
            0 => chunk.index = 1,
            1 => chunk.offset = 1,
    ...
    var transfer = try zigrock.transfer.Transfer.init(std.testing.allocator, m, .{});
    defer transfer.deinit();
    for (0..2) |_| try transfer.accept(try zigrock.transfer.Transfer.serve(m, archive, try transfer.request(), .{}));
    try std.testing.expectError(error.IntegrityMismatch, transfer.accept(try zigrock.transfer.Transfer.serve(m, archive, try transfer.request(), .{})));
