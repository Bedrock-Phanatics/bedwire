...
test "manifest reuses bounded protocol codec and validates truncation" {
    ...
    try n.protocol.codecs.resource_pack.encodeInfo(&writer, .{ .texture_pack_required = false, .has_addons = false, .has_scripts = false, .force_disable_vibrant_visuals = false, .world_template_uuid = @splat(0), .world_template_version = "", .texture_packs = &.{} });
    var manifest = try n.resource_packs.Manifest.decode(std.testing.allocator, writer.written(), .{});
    defer manifest.deinit();
    try std.testing.expectEqual(@as(usize, 0), manifest.info.texture_packs.len);
    for (0..writer.cursor) |end| {
        if (n.resource_packs.Manifest.decode(std.testing.allocator, bytes[0..end], .{})) |value| {
            var owned = value;
            owned.deinit();
...
fn allocationCase(allocator: std.mem.Allocator) !void {
    var negotiation = try n.resource_packs.Negotiation.init(allocator, &.{ "a", "b" }, .{});
    defer negotiation.deinit();
}

test "pack negotiation forbids unknown, duplicate, and premature completion" {
    try std.testing.checkAllAllocationFailures(std.testing.allocator, allocationCase, .{});
    var negotiation = try n.resource_packs.Negotiation.init(std.testing.allocator, &.{ "a", "b" }, .{});
    defer negotiation.deinit();
...
fn decodeCase(allocator: std.mem.Allocator, bytes: []const u8) !void {
    var manifest = try n.resource_packs.Manifest.decode(allocator, bytes, .{});
    defer manifest.deinit();
...
test "nonempty manifest allocation failures, truncation and resource caps" {
    var packs = [_]n.protocol.packets.resource_pack.TexturePackInfo{.{ .uuid = @splat(1), .version = "1.0.0", .size = 10, .content_key = "", .sub_pack_name = "", .content_identity = "", .has_scripts = false, .addon_pack = false, .rtx_enabled = false, .download_url = "" }};
    var bytes: [256]u8 = undefined;
    var writer = n.protocol.Writer.init(&bytes);
    try n.protocol.codecs.resource_pack.encodeInfo(&writer, .{ .texture_pack_required = true, .has_addons = false, .has_scripts = false, .force_disable_vibrant_visuals = false, .world_template_uuid = @splat(0), .world_template_version = "", .texture_packs = &packs });
    try std.testing.checkAllAllocationFailures(std.testing.allocator, decodeCase, .{writer.written()});
    try std.testing.expectError(error.LimitExceeded, n.resource_packs.Manifest.decode(std.testing.allocator, writer.written(), .{ .max_resource_pack_bytes = 9 }));
    for (0..writer.cursor) |end| {
        if (n.resource_packs.Manifest.decode(std.testing.allocator, bytes[0..end], .{})) |value| {
            var owned = value;
            owned.deinit();
