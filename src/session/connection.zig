...
pub fn Connection(comptime Carrier: type) type {
    comptime @import("../transport/carrier.zig").validate(Carrier);
    return struct {
        const Self = @This();
        ...
        pub fn init(allocator: std.mem.Allocator, carrier: *Carrier, role: state.Role, limits: Limits) !Self {
            ...
            const encoded_capacity = try @import("../compression/snappy.zig").maxEncodedLen(limits.protocol.max_decompressed_batch_bytes);
            ...
            return .{ .allocator = allocator, .carrier = carrier, .role = role, .limits = limits, .ingress = ingress, .outgoing = outgoing, .batch_buffer = batch_buffer, .encoder = encoder, .decoder = decoder };
        }
        ...
        pub fn receive(self: *Self) !protocol.batch.Iterator {
            ...
            errdefer self.close();
            const exclusive = self.state == .handshake or self.state == .authenticating or self.state == .encrypted_handshake;
            if (exclusive and self.pending_id != null) return error.InvalidState;
            ...
            while (try iterator.next()) |packet| {
                ...
                const subclient_handshake = self.state != .in_game and (envelope.header.sender_subclient != 0 or envelope.header.target_subclient != 0);
            ...
            const single_packet_state = self.state == .handshake or self.state == .authenticating or self.state == .encrypted_handshake;
        ...
        pub fn send(self: *Self, packets: []const []const u8) !void {
            ...
            const payload = switch (algorithm) {
                ...
                .snappy => try @import("../compression/snappy.zig").compress(raw, self.encoder.output, &self.encoder.snappy_table),
            };
            ...
            if (packets.len != 0) self.last_sent_id = (try protocol.packet.decode(packets[packets.len - 1], self.limits.protocol)).header.packet_id;
        }
        ...
        pub fn enableCompression(self: *Self, algorithm: compression.Algorithm, threshold: u16) !void {
            ...
            const allowed = self.state == .handshake and self.pending_id == expected and algorithm != .none and settings_sent;
        ...
        pub fn authenticateLegacy(self: *Self, allocator: std.mem.Allocator, chain_json: []const u8, client_data: []const u8, policy: @import("../auth/chain.zig").Policy) !@import("../auth/chain.zig").AuthData {
        ...
        pub fn authenticateOidc(self: *Self, allocator: std.mem.Allocator, token: []const u8, client_data: []const u8, policy: @import("../auth/chain.zig").OidcPolicy) !@import("../auth/chain.zig").AuthData {
            if (self.state != .authenticating or self.pending_id != 1) return error.InvalidState;
            errdefer self.close();
        ...
        pub fn installServerCrypto(self: *Self, secret: spki.Ecdsa.SecretKey, salt: [16]u8) !void {
            const allowed = self.role == .server and self.state == .authenticating and self.pending_id == 1 and self.last_sent_id == 3;
            if (!allowed) return error.InvalidState;
            const peer = self.verified_key orelse return error.Unauthenticated;
            var key = try @import("../crypto/ecdh.zig").derive(secret, peer, salt);
        ...
        pub fn acceptServerHandshake(self: *Self, allocator: std.mem.Allocator, encoded: []const u8, secret: @import("../crypto/spki.zig").Ecdsa.SecretKey) !void {
            if (self.role != .client or self.state != .authenticating or self.pending_id != 3) return error.InvalidState;
            errdefer self.close();
            var token = try @import("../auth/jwt.zig").Token.parse(allocator, encoded, self.limits);
            defer token.deinit();
            const peer = try @import("../crypto/spki.zig").fromBase64(try @import("../auth/jwt.zig").string(token.header.value, "x5u"));
            try token.verify(peer);
            const encoded_salt = try @import("../auth/jwt.zig").string(token.payload.value, "salt");
